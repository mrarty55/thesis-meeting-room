...
import { generateRentReport } from '~/utils/pdf'
export default {
  layout: 'admin',
  data() {
    return {
  ...
  computed: {
    rents() {
      return this.$store.state.admin.report.rent.rents
    },
  },
  mounted() {
    this.$store.commit('ui/setAppBarTitle', this.title)
    this.$store.dispatch('admin/report/rent/getRentReport')
  },
  methods: {
    onExportAsPdf() {
      generateRentReport(this.rents)
    },
  },
}
</script>
